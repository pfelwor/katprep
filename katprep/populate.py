...
"""
Populates the Foreman/Katello or Red Hat Satellite 6 host parameters with information from
Nagios/Icinga or a virtualization host
"""
...
def populate(options):
    """
    Retrieves information from a virtualization system and tries to merge data with Foreman
    """
...
def parse_options(args=None):
    ...
    # -C / --auth-container
    gen_opts.add_argument(
        "-C", "--auth-container", default="", metavar="FILE", dest="generic_auth_container",
        action="store", help="defines an authentication container file (default: no)"
    )
    # -P / --auth-password
    gen_opts.add_argument(
    ...
    # --ip-filter
    gen_opts.add_argument(
        "--ipv6-only", dest="ipv6_only", default=False, action="store_true",
        help="Filters for IPv6-only addresses (default: no)"
    )
    # --insecure
    gen_opts.add_argument(
    ...
    # -u / --update
    fman_opts.add_argument(
        "-u", "--update", dest="foreman_update", action="store_true", default=False,
        help="Updates pre-existing host parameters (default: no)"
    )

    # VIRTUALIZATION ARGUMENTS
    virt_opts.add_argument(
        "--virt-uri", dest="virt_uri", metavar="URI", default="", help="defines an URI to use"
    )
    # --virt-type
    virt_opts.add_argument(
        "--virt-type", dest="virt_type", metavar="libvirt|pyvmomi", default="libvirt", type=str,
        help="defines the library used to operate with virtualization host: libvirt or pyvmomi "
             "(vSphere). (default: libvirt)"
    )
    # --skip-virt
    virt_opts.add_argument(
        "--skip-virt", dest="virt_skip", default=False, action="store_true",
        help="skips gathering data from virtualization host (default: no)"
    )
    ...
    # --mon-type
    mon_opts.add_argument(
        "--mon-type", dest="mon_type", metavar="nagios|icinga", type=str, choices="nagios|icinga",
        default="icinga", help="defines the monitoring system type: nagios (Nagios/Icinga 1.x) or "
                               " icinga (Icinga 2.x). (default: icinga)"
    )
    # --skip-mon
    mon_opts.add_argument(
        "--skip-mon", dest="mon_skip", default=False, action="store_true",
        help="skips gathering data from monitoring system (default: no)"
    )
